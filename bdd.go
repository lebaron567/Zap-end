...
func AddLikeAndDislike(id_post int, id_user int, effect string) error {
	...
	rows, err := database.Query(`SELECT effect FROM like WHERE id_user = `+strconv.Itoa(id_user)+` AND id_post = `+strconv.Itoa(id_post)+` ;`)//get like if existe, if not créate new like and if value need to be change it change the value 
	if err != nil {
		log.Fatal(err)
	}
	defer database.Close()
